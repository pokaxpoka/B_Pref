...
class BabyEnvironment(dm_env.Environment):
    """Class for physics-based reinforcement learning environments."""
    ...
    def get_mode_flag(self):
        return self._task.get_mode_flag()
    
    def set_mode_flag(self, mode):
        return self._task.set_mode_flag(mode)
    
    def get_reward_thres(self):
        return self._task.get_reward_thres()
    
    def set_reward_thres(self, reward_th):
        return self._task.set_reward_thres(reward_th)
...
def compute_n_steps(control_timestep, physics_timestep, tolerance=1e-8):
    ...
    if control_timestep < physics_timestep:
        raise ValueError(
            'Control timestep ({}) cannot be smaller than physics timestep ({}).'.format(control_timestep, physics_timestep))
    if abs((control_timestep / physics_timestep - round(control_timestep / physics_timestep))) > tolerance:
        raise ValueError('Control timestep ({}) must be an integer multiple of physics timestep ({})'.format(control_timestep, physics_timestep))
